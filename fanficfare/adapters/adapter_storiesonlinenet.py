    ...
    def findStoryRow(self, row_class='tr'):
        ...
        while not story_found:
            ...
            if 'css/t-classic2.css' not in data and self.getConfig("force_classic_theme",True):
                ### XXX - site 'fixed' their issue with user not being
                ### able to reliably change theme back to Classic.
                ### however, it doesn't appear that changing it here
                ### works.  Don't know why not.
                ...
                data=self._fetchUrl('https://'+self.getSiteDomain()+'/sol-secure/user/my_account.php',usecache=False)
                data=self._fetchUrl('https://'+self.getSiteDomain()+'/sol-secure/user/my_account.php?cmd=listingFonts',usecache=False)
                logger.debug(data)
                settingssoup = self.make_soup(data)
                classic_val = settingssoup.find('option',text="Classic")['value']
                logger.debug("classic_val:%s"%classic_val)
                postUrl = 'https://'+self.getSiteDomain()+'/sol-secure/user/my_account.php'
                params = {'listingTheme':classic_val,
                          'listingSize':10,
                          'listing_rows':50,
                          'listingProf':'auto',
                ...
                data = self._fetchUrl(self.story.getList('authorUrl')[0] + "/" + unicode(page))
                logger.debug("author page after:==================================================")
                logger.debug(data)
